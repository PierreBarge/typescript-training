describe("arrow functions", () => {
  it("can replace simple traditional functions", () => {
    ...
    let fnAdd = function (a: number, b: number) {
      ...
      arrowAdd = (a: number, b: number) => a + b;
  ...
  describe("are great for defining simple calculations", () => {
    ...
    let arrowSub = (a: number, b: number) => a - b,
      arrowMul = (a: number, b: number) => a * b,
      arrowDiv = (a: number, b: number) => a / b;
  ...
  it("can replace complex traditional functions", () => {
    ...
    let fnFib = function (x: number) {
      ...
      arrowFib = (x: number) =>
        x === 0 ? 0 : x === 1 ? 1 : arrowFib(x - 1) + arrowFib(x - 2);
  ...
  it("binds `this` to the eval scope, not the runtime scope", () => {
    ...
    const person = {
      name: "Jarosław",
      greetFriends: function (friends: string[]) {
  ...
  it("can return objects", () => {
    ...
    let makePerson = (firstname: string, lastname: string) => {
      return {
        first: firstname,
        last: lastname,
      };
    ...
    expect(makePerson("John", "Lennon")).toEqual({
      first: "John",
      last: "Lennon",
    });
    expect(makePerson("Paul", "McCartney")).toEqual({
      first: "Paul",
      last: "McCartney",
    });
  });
  ...
  it("are great for currying", () => {
    ...
    let adder = (first: number) => (second: number) => first + second,
      add5 = adder(5),
      minus10 = adder(-10);
  ...
  it("can make array filter chains more managable", () => {
    const data = [
      { type: "Clothes", name: "Socks", price: 1.0, qty: 5 },
      { type: "Clothes", name: "Trousers", price: 3.9, qty: 2 },
      { type: "Clothes", name: "Pajams", price: 4.8, qty: 1 },
      { type: "Clothes", name: "Shoes", price: 23.0, qty: 2 },
      { type: "Music", name: "David Bowie", price: 11.9, qty: 1 },
      { type: "Music", name: "Rolling Stones", price: 8.9, qty: 1 },
      { type: "Music", name: "ABBA", price: 9.9, qty: 1 },
      { type: "Food", name: "Chips", price: 3.5, qty: 4 },
      { type: "Food", name: "Fish", price: 8.75, qty: 3 },
      { type: "Food", name: "Beer", price: 2.8, qty: 12 },
    ];
