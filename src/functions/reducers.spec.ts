...
describe("Reducers", () => {
  ...
  it("can split one big collection into smaller grouped collections", () => {
    ...
    const groupedAggregate = data.reduce((acc, item) => {
      if (!acc[item.type]) {
        acc[item.type] = [];
      }
  ...
  it("can also apply calculations to grouped items", () => {
    ...
    let priceAggregate = data.reduce((acc, item) => {
      if (!acc[item.type]) {
        acc[item.type] = 0;
      }
      acc[item.type] = acc[item.type] + item.qty * item.price;
      return acc;
    }, {});
  ...
  it("can check if all predicates are truthy", () => {
    ...
    function allTruthy(value: number, predicates: Function[]): boolean {
      return predicates.reduce((bool: boolean, currentFunction: Function) => {
        if (bool && currentFunction(value)) {
          return true;
        } else {
          return false;
        }
  ...
  it("sequential processing via function pipe", () => {
    ...
    const execute = (operationsArray: Function[], startingValue: number) =>
      operationsArray.reduce(
        (total: number, currentOperation: Function) => currentOperation(total),
        startingValue
      );
