...
describe("Immutable ES6 operations", () => {
  ...
  it("merge two objects", () => {
    ...
    const merge2objects = (obj1: object, obj2: object): object => {
      const newObject = {};
  ...
  it("merging multiple objects", () => {
    ...
    const mergeManyObjects = (...args: object[]): object => {
      const newObject = {};
      for (let i = 0; i <= args.length; i++) {
        for (let prop in args[i]) {
          newObject[prop] = args[i][prop];
        }
      }
      return newObject;
    };
  ...
  it("strip static attribute from objects", () => {
    ...
    const stripId = (obj: object): object => {
      const newObject = {};
      for (let prop in obj) {
        if (prop !== "id") {
  ...
  it("strip dynamic attribute from objects", () => {
    ...
    const stripKey = (key: string, obj: object): object => {
      const newObject = {};
      for (let prop in obj) {
        if (prop !== key) {
          newObject[prop] = obj[prop];
    ...
    expect(stripKey("qty", stripKey("price", shoppingData[0]))).toEqual({
      type: "Clothes",
      name: "Socks",
      id: 421801449988,
    });
