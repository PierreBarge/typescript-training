...
describe("Functional Programming", function () {
  describe("always function", function () {
    it("returns the same value no matter the arguments", function () {
      ...
      // function always
      const always = (value: any) => {
        const eternalValue = value;
        return (...args: any[]) => eternalValue;
      };
  ...
  describe("unary function", function () {
    it("can override arguments passed down to a function", function () {
      ...
      expect(["10", "10", "10", "10"].map(unaryParseInt)).toEqual([
        10, 10, 10, 10,
      ]);
    });
  });
  ...
  describe("compose function", function () {
    ...
    it("can execute sequence of N operations as a new reusable function", function () {
      ...
      function compose(...operations) {
        ...
        return (originalName: string) => {
          let fullName: string = "";
          for (let orderOp of orderedOperations) {
            if (operations.includes(orderOp)) {
              fullName === ""
                ? (fullName = orderOp(originalName))
                : (fullName = orderOp(fullName));
            }
          }
          return fullName;
        };
      ...
      var appendMr = function (str) {
        return "Mr. " + str;
      };
      var appendJr = function (str) {
        return str + " Jr";
      };
      var appendThird = function (str) {
        return str + " III";
      };
      var appendTheMad = function (str) {
        return str + " the Mad";
      };
      var appendDukeOfEscobar = function (str) {
        return str + ", Duke of Escobar";
      };
      var thirdTheMadDukeOfEscobar = compose(
        appendDukeOfEscobar,
        appendTheMad,
        appendThird
      );
      expect(thirdTheMadDukeOfEscobar("John Lennon")).toEqual(
        "John Lennon III the Mad, Duke of Escobar"
      );
      expect(thirdTheMadDukeOfEscobar("Paul McCartney")).toEqual(
        "Paul McCartney III the Mad, Duke of Escobar"
      );
      var mrJrTheThird = compose(appendMr, appendThird, appendJr);
      expect(mrJrTheThird("John Lennon")).toEqual("Mr. John Lennon Jr III");
      expect(mrJrTheThird("Paul McCartney")).toEqual(
        "Mr. Paul McCartney Jr III"
      );
    });
    ...
    it("can be used for complex data processing", function () {
      ...
      function getEmployeesByNationality(nationality: string) {
        return (array: { nationality: string }[]) =>
          array.filter((item) => item.nationality === nationality);
      }
      ...
      function getSalaries(array: { salary: number }[]) {
        return array.map((item) => item.salary);
      }

      expect(getSalaries([{ salary: 125 }, { salary: 345 }])).toEqual([
        125, 345,
      ]);
      ...
      function compose(...operations) {
        ...
        return (startingValue) => {
          for (let operation of reversedOps) {
            if (result === undefined) {
              result = operation(startingValue);
            } else {
  ...
  describe("method function", function () {
    ...
    it("can invoke a certain method on whatever object you pass", function () {
      ...
      function method(methodName: string): Function {
        return (obj: { hello: Function }) => {
