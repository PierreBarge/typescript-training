describe("enhanced object literals", () => {
  it("can use shorthand for property names", () => {
    function createBeatle(name, dead) {
      ...
      return {
        ...
        wantsToPlayWith(target) {
          return `${this.name} wants to play with ${target.name} ${
            target.dead ? "but he is" : "and he is not"
          } dead`;
        },
        // - in ES5 you'd do:
        // name: name,
    ...
    expect(paul.wantsToPlayWith(john)).toBe(
      "Paul wants to play with John but he is dead"
    );
    expect(paul.wantsToPlayWith(george)).toBe(
      "Paul wants to play with George and he is not dead"
    );
  });
});
