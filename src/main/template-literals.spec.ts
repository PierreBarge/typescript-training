describe("Template literals", () => {
  ...
  it("should support string escaping", () => {
    ...
    const one = "Hi\nthere!";
    const two = "This is `escaped` backtics";

    expect(one).toBe("Hi\nthere!");
    expect(two).toBe("This is `escaped` backtics");
  });
  ...
  it("should call the tagging function", () => {
    ...
    expect(result).toBe(
      "Welcome dear John, feel comfortable and really take a seat!"
    );
  ...
  it("can be curried", () => {
    ...
    let journey = (string1) => (string2) => (string3) =>
      `${string1}, then ${string2} and finally ${string3}!`;

    expect(journey`Warsaw``Poznan``Berlin`).toBe(
      "Warsaw, then Poznan and finally Berlin!"
    );
    expect(journey`Poland``Czech``Austria`).toBe(
      "Poland, then Czech and finally Austria!"
    );
    expect(journey`Europe``Asia``Australia`).toBe(
      "Europe, then Asia and finally Australia!"
    );
  });
});
