...
type Nationality = "UK" | "DE" | "FR" | "US" | "IT" | "ES" | "PL";
...
export const db = (function () {
  ...
  const nationalities: Nationality[] = ["US", "UK", "DE", "FR"];
  ...
  const getTodoById: (id: string) => Todo = (id) =>
    data.todos.find((el) => el.id === id);

  const getTodos: () => Todo[] = () => data.todos;

  const getEmployeeById: (id: number) => Employee = (id) =>
    data.employees.find((el) => el.id === id);

  const getEmployees: () => Employee[] = () => data.employees;

  const getEmployeesByNationality: (nat: Nationality) => Employee[] = (nat) =>
    data.employees.filter((el) => el.nationality === nat);

  const getNationalities: () => Nationality[] = () => data.nationalities;

  const getAllTransactions: () => Transaction[] = () => data.transactions;

  const getTransactionById: (id: number) => Transaction = (id) =>
    data.transactions.find((el) => el.id === id);

  const submitTransaction: (transaction: Transaction) => void = (trans) =>
